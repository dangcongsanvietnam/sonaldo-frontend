...
import { Badge, Breadcrumb, Button, Card, Checkbox, ConfigProvider, Drawer, Input, InputNumber, Modal, Rate, Select, Slider, Spin } from "antd";
...
import { getCategoryDetail, getCategoryItemDetail } from "../../services/categoryService";
...
const CategoryItemPage = () => {
    ...
    useEffect(() => {
        dispatch(getCategoryDetail(categoryId)).unwrap();
    ...
    const fetchData = async () => {
        ...
        try {
            await dispatch(getAdminBrands()).unwrap().then((res) => {
                ...
                fetchCategoryItemDetail(brands);
            })
        } catch (error) {
    ...
    const fetchCategoryItemDetail = async (brandsData) => {
        startLoading(true);
        try {
            const res = await dispatch(getCategoryItemDetail({ categoryId, categoryItemId })).unwrap();

            const categoryItem = {
                ...res.data,
                name: getLocalizedText(res.data.name),
                description: getLocalizedText(res.data.description),
            };

            setCategoryItemData(categoryItem);

            const productPromises = async () => {
                const response = await dispatch(getProductsByCategoryItem(categoryItemId)).unwrap();
                ...
                return {
                    ...
                    products: response.data.map((product) => ({
                        ...
                        categoryItems: product.categoryItems?.map((categoryItem) => ({
                            ...
                            name: getLocalizedText(categoryItem?.name),
                            categoryName: getLocalizedText(categoryItem?.categoryName),
                            categoryId: categoryItem.categoryItemId, // Ensure correct categoryId
                        })),
            ...
            const categoryProducts = await productPromises();
            ...
            const flattenedCategoryItems = allProductsArray.flatMap((product) =>
                product.categoryItems?.filter((categoryItem) =>
                    getLocalizedText(categoryItem.categoryName) !== "Mua Sắm Theo Độ Tuổi" &&
                    getLocalizedText(categoryItem.categoryName) !== "Shop By Age"
                ) || []
            );

            flattenedCategoryItems.forEach((categoryItem) => {
                const existing = brandCategoryMap.get(categoryItem.categoryId);
                brandCategoryMap.set(categoryItem.categoryId, {
            ...
            const brandListFormatted = brandsData
                ?.map((brand) => {
                    ...
                    const count = allProductsArray.reduce(
                        (acc, product) =>
                            matchingBrandCategories?.some((category) => category.brandCategoryId === product.brandCategory?.brandCategoryId)
                                ? acc + 1
                                : acc,
                        0
                    );
    ...
    const handleLoginOrEmailSubmit = (email) => {
        if (email) {
            // Perform logic to save the email and continue adding to favorites
            console.log("User provided email:", email);
            message.success("We will notify you!");
        } else {
            // Handle login case
            console.log("User decided to log in");
        }
        ...
        >
            <div className="p-6 max-w-screen-xl mx-auto">
                ...
                >
                    ...
                    <Breadcrumb.Item>
                        <span onClick={() => navigate(`/category/${categoryId}`)} className="cursor-pointer text-sm text-[#1F4ABC] underline">
                            {category?.name || "Category"}
                        </span>
                    </Breadcrumb.Item>
                    <Breadcrumb.Item>
                        <span onClick={() => navigate(`/category/${categoryId}/${categoryItemId}`)} className="cursor-pointer text-sm text-[#1F4ABC] underline">
                            {categoryItemData?.name || "Category"}
                        </span>
                    </Breadcrumb.Item>
                </Breadcrumb>
                <h1 style={{
                    fontSize: "50px",
                    marginBottom: "10px"
                }}>{categoryItemData?.name}</h1>
                <p style={{
                    marginBottom: "20px"
                }}>{categoryItemData?.description}</p>
